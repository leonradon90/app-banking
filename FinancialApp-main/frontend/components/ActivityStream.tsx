    ...
    <div className="glass-panel shadow-panel rounded-3xl border border-[var(--border)] bg-white/90 p-6">
      ...
      {events.length === 0 ? (
        <p className="mt-4 rounded-2xl border border-dashed border-[var(--border)] bg-white/60 p-4 text-xs text-[var(--muted)]">
          No activity yet. Transfers and card actions will appear here.
        </p>
      ) : (
            ...
            <li key={event.id} className="rounded-2xl border border-transparent bg-white/60 p-4 shadow-sm">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  {event.actor && <p className="mt-1 text-xs text-brand-secondary/70">User: {event.actor}</p>}
                </div>
                <span className={statusStyles[event.status]}>
                  {event.status === 'success' ? 'Success' : event.status === 'pending' ? 'Pending' : 'Blocked'}
                </span>
              </div>
