...
export class FraudService {
  ...
  async validatePayment(
    ...
    await this.auditService.record(actor, passed ? 'FRAUD_CHECK_PASSED' : 'FRAUD_CHECK_FAILED', {
      accountId,
      amount: dto.amount,
    ...
    if (!passed) {
      throw new BadRequestException(
        `Transaction rejected due to high fraud risk. Risk score: ${riskScore}%. Reasons: ${reasons.join(', ')}`,
      );
    }
